...
export interface AssignAttachmentDto {
  studentId: string;
  supervisorId: string;
  companyId?: string;
  startDate: string;
...
// ============================================================
export const assignAttachmentService = async (
  assignedBy: string,
  dto: AssignAttachmentDto
) => {
  ...
  const ongoing = await db
    ...
    .where(
      and(
        eq(attachments.studentId, dto.studentId),
...
// ============================================================
export const getAttachmentByIdService = async (attachmentId: string, supervisorId?: string) => {
...
// ============================================================
export const getMyAttachmentService = async (studentId: string) => {
  const result = await db
    .select({
      attachmentId:    attachments.id,
      status:          attachments.status,
    ...
    .where(
      and(
        eq(attachments.studentId, studentId),
        eq(attachments.status, "ongoing")
      )
    )
    .limit(1);
...
// ============================================================
export const endAttachmentService = async (attachmentId: string) => {
