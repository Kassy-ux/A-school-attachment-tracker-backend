...
// CHECK IN
// ============================================================
export const checkInService = async (studentId: string) => {
...
// CHECK OUT
// ============================================================
export const checkOutService = async (studentId: string) => {
...
// GET TODAY'S STATUS
// ============================================================
export const getTodayStatusService = async (studentId: string) => {
  const today = new Date().toISOString().split("T")[0];
...
// ============================================================
export const getAttendanceSummaryService = async (studentId: string) => {
  const [summary] = await db
    .select({
      totalDays: count(),
      totalHours: sql<number>`coalesce(sum(${attendance.totalHours}), 0)`,
      completedDays: sql<number>`count(*) filter (where ${attendance.checkOut} is not null)`,
    })
    .from(attendance)
    .where(eq(attendance.studentId, studentId));
